...
[Получить Yandex API key](https://developer.tech.yandex.ru/services/) для получения координат по адресу



Настроить бэкенд: создать файл `.env` в каталоге `star_burger/` со следующими настройками:
- `YANDEX_KEY` - Ключ Яндекса
...
- Информация о данном сервере:
   - ip ```188.68.222.57```
   - Домен ```sstarbburger.ru```
   - Имя пользователя ```root```
   - Деплойный скрипт ```~/opt/deploy_star_burger.sh```
